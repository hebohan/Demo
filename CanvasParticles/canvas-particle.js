var CanvasParticle = (function(){
	...
	// 根据传入的config初始化画布
	function canvasInit(div_id, canvasConfig){
		canvasConfig = canvasConfig || {};
		var html = getElementByTag("html")[0];
		var body = getElementByTag("body")[0];		
		var canvasDiv = getELementById(div_id);
		...
		var canvas = {
			...
			config: {
				...
				max_conn: 10,
				is_mouse: canvasConfig.is_mouse || false
			}
		};
		...
		canvasDiv.appendChild(canvasObj);
	}
	...
	function drawLine(context, canvas, mouse){
		...
		for(var i = 0, len = canvas.config.count; i < len; i++){
			...
			// 如果鼠标进入画布
			// point to mouse
			var aaa=canvas.config.is_mouse;
			if(canvas.config.is_mouse){
				if(mouse){
